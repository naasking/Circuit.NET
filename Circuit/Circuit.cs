...
namespace Circuit
{
    ...
    public struct Circuit<T0, T1>
    {
        ...
        internal Action<T0, Queue<Action>, Action<T1>> k;

        internal Circuit(Action<T0, Queue<Action>, Action<T1>> k) =>
            this.k = k;
        ...
        {
            var k = this.k;
            return new Circuit<T0, T2>((x0, q, k2) => k(x0, q, x1 => next.k(x1, q, x2 => k2(x2))));
        }
        ...
        {
            var k = this.k;
            return new Circuit<T0, T2>((x0, q, k2) => k(x0, q, x1 => k2(next(x1))));
        }
        ...
        /// <param name="next"></param>
        /// <returns></returns>
        public Circuit<T0, T1> Where(Func<T1, bool> predicate)
        {
            var k = this.k;
            return new Circuit<T0, T1>((x0, q, k2) => k(x0, q, x1 =>
            {
                if (predicate(x1))
                    k2(x1);
            }));
        ...
        {
            var k = this.k;
            return new Circuit<T0, (T1, T2)>((x0, q, k12) => next.k(x0, q, x2 => k(x0, q, x1 => k12((x1, x2)))));
        }
        ...
        public Circuit<T0, T3> Merge<T2, T3>(Circuit<T0, T2> next, Func<T1, T2, T3> select)
        {
            ...
            var k = this.k;
            return new Circuit<T0, T3>((x0, q, k12) => next.k(x0, q, x2 => k(x0, q, x1 => k12(select(x1, x2)))));
        }
        ...
        {
            var k = this.k;
            return new Circuit<T0, T1>((x0, q, k1) => q.Enqueue(() => k(x0, q, k1)));
        }
        ...
        {
            var k = this.k;
            return new Circuit<(T0, T2), (T1, T2)>((x02, q, k12) => k(x02.Item1, q, x1 => k12((x1, x02.Item2))));
        }
        ...
        {
            var q = new Queue<Action>(2);
            this.k?.Invoke(input, q, output);
            while (q.Count > 0)
                q.Dequeue().Invoke();
        }
        ...
        {
            var circ = this;
            register((o, e) => circ.Run(e, output));
        }
    ...
    public static class Circuit
    {
        ...
        public static Circuit<T0, T1> Create<T0, T1>(Func<T0, T1> init) =>
            new Circuit<T0, T1>((x0, q, k1) => k1(init(x0)));
        ...
        public static Circuit<Empty, T> Constant<T>(T value) =>
            new Circuit<Empty, T>((_, q, k) => k(value));
        ...
        public static Circuit<Empty, T> Sampler<T>(Func<T> sampler) =>
            new Circuit<Empty, T>((_, q, k) => k(sampler()));
        ...
        {
            return new Circuit<T0, Event<T1>>((x0, q, k1) => first.k(x0, q, x1 => second.k(x0, q, x2 => k1(x1.HasValue ? x1 : x2))));
        }
        ...
        {
            Action<T0, Queue<Action>, Action<T1>> k = null;
            return new Circuit<T0, T1>((x0, q, k1) => source.k(x0, q, x1e2 =>
            {
        ...
        {
            return new Circuit<T0, T1>((x0, q, k1) => source.k((x0, x2), q, x12 =>
            {
                x2 = x12.Item2;
                k1(x12.Item1);
            }));
